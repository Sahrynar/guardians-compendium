...
export function useDB() {
  ...
  // ── Full Compendium Markdown Export ──────────────────────────
  const exportMarkdown = useCallback(() => {
    const lines = []
    const now = new Date().toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })
    
    lines.push('# The Guardians of Lajen — Worldbuilding Compendium')
    lines.push(`*Exported ${now}*`)
    lines.push('')
    lines.push('---')
    lines.push('')

    // Section configs: what to include and how to format each
    const sections = [
      { key: 'characters',  title: 'Characters',       nameField: 'name', fields: [
        ['element','Element'],['birthday','Birthday (Mnaerah)'],['age_b1','Age B1'],
        ['traits','Traits'],['notes','Notes'],['aliases','Aliases'],
      ]},
      { key: 'locations',   title: 'Locations',        nameField: 'name', fields: [
        ['loc_type','Type'],['description','Description'],['notes','Notes'],
      ]},
      { key: 'scenes',      title: 'Scenes',           nameField: 'name', fields: [
        ['book','Book'],['chapter','Chapter'],['characters_present','Characters'],
        ['location_id','Location'],['summary','Summary'],['notes','Notes'],
      ]},
      { key: 'timeline',    title: 'Timeline',         nameField: 'name', fields: [
        ['date_hc','Date (HC)'],['era','Era'],['detail','Detail'],
      ]},
      { key: 'canon',       title: 'Canon Decisions',  nameField: 'name', fields: [
        ['session','Session'],['detail','Detail'],['notes','Notes'],
      ]},
      { key: 'world',       title: 'World Entries',    nameField: 'name', fields: [
        ['detail','Detail'],['notes','Notes'],
      ]},
      { key: 'questions',   title: 'Open Questions',   nameField: 'name', fields: [
        ['detail','Detail'],['notes','Notes'],
      ]},
      { key: 'flags',       title: 'Flags',            nameField: 'name', fields: [
        ['priority','Priority'],['detail','Detail'],
      ]},
      { key: 'spellings',   title: 'Spellings',        nameField: 'name', fields: [
        ['correct','Correct'],['notes','Notes'],
      ]},
      { key: 'inventory',   title: 'Inventory',        nameField: 'name', fields: [
        ['category','Category'],['character','Holder'],['description','Description'],
      ]},
      { key: 'wardrobe',    title: 'Wardrobe',         nameField: 'name', fields: [
        ['character','Character'],['item_type','Type'],['description','Description'],
      ]},
      { key: 'items',       title: 'Items',            nameField: 'name', fields: [
        ['item_type','Type'],['holder','Holder'],['description','Description'],
      ]},
      { key: 'wiki',        title: 'Wiki Articles',    nameField: 'title', fields: [
        ['category','Category'],['summary','Summary'],
      ]},
      { key: 'notes',       title: 'Notes',            nameField: 'title', fields: [
        ['content','Content'],
      ]},
      { key: 'manuscript',  title: 'Manuscript',       nameField: 'title', fields: [
        ['book','Book'],['chapter_num','Chapter'],['status','Status'],
        ['word_count','Words'],['notes','Notes'],
      ]},
      { key: 'maps',        title: 'Maps',             nameField: 'name', fields: [
        ['notes','Notes'],
      ]},
    ]

    sections.forEach(({ key, title, nameField, fields }) => {
      const entries = (db[key] || []).filter(e => e[nameField] || e.name)
      ...
      entries.forEach(e => {
        const name = e[nameField] || e.name || '(unnamed)'
        const status = e.status ? ` · ${e.status}` : ''
        const books = (e.books || []).length ? ` · ${e.books.join(', ')}` : e.book ? ` · ${e.book}` : ''
        lines.push(`### ${name}${status}${books}`)
        
        fields.forEach(([field, label]) => {
          const val = e[field]
          if (!val || (Array.isArray(val) && !val.length)) return
          const display = Array.isArray(val) ? val.join(', ') : String(val)
          if (display.trim()) lines.push(`**${label}:** ${display}`)
        })
        ...
        if (ts) {
          try {
            const d = new Date(ts).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })
    ...
    lines.push('*End of Compendium export*')

    const blob = new Blob([lines.join('\n')], { type: 'text/markdown' })
    const a = document.createElement('a')
    a.href = URL.createObjectURL(blob)
    a.download = `guardians_compendium_${new Date().toISOString().slice(0,10)}.md`
    a.click()
  ...
  return {
    ...
    exportJSON, importJSON, importAster, exportAster, exportMarkdown,
