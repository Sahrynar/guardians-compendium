  ...
  return (
    <div style={{ marginBottom: 2 }}>
      <div className="loc-node" onClick={() => onToggle(loc.id)}>
        {kids.length > 0
          ? <span style={{ fontSize: '0.69em', color: '#ffb700', transition: '.2s', display: 'inline-block', transform: isOpen ? 'rotate(90deg)' : 'none' }}>►</span>
          : <span style={{ width: 12 }} />
        }
      ...
      {isOpen && (
        <div style={{ padding: '6px 8px 6px 24px', fontSize: '0.85em', color: 'var(--dim)' }}>
          ...
          <div className="entry-actions" style={{ marginTop: 4 }}>
            <button className="btn btn-sm btn-outline" style={{ color: '#ffb700', borderColor: '#ffb700' }} onClick={e => { e.stopPropagation(); onEdit(loc) }}>✎</button>
            <button className="btn btn-sm btn-outline" style={{ color: '#ffb700', borderColor: '#ffb700' }} onClick={e => { e.stopPropagation(); onAddChild(loc.id) }}>+ Child</button>
...
export default function Locations({ db }) {
  ...
  return (
    <div>
      <div className="tbar">
        <div style={{ fontFamily: "'Cinzel', serif", fontSize: '1.15em', color: '#ffb700' }}>🗺 Locations</div>
        <button className="btn btn-primary btn-sm" style={{ background: '#ffb700', color: '#000' }} onClick={() => openAdd()}>+ Add</button>
      </div>
