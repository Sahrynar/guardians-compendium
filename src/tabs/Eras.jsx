...
import { ERA_TIMELINE, ERA_SPANS, uid } from '../constants'

// Rows live in the database (category 'eras'): kind 'tl' = timeline rows, kind 'span' = era-span cards.
// One-time seed: old settings blobs if present, otherwise the built-in defaults.
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 400, background: 'rgba(0,0,0,.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div style={{ background: 'var(--sf)', border: '1px solid var(--brd)', borderRadius: 12, padding: 20, width: '100%', maxWidth: 420, maxHeight: '90vh', overflowY: 'auto' }}>
        <div style={{ fontFamily: "'Cinzel',serif", fontSize: '1.08em', color: 'var(--cca)', marginBottom: 14 }}>
          ✎ {item.id ? 'Edit' : 'Add'} {type === 'tl' ? 'Timeline Entry' : 'Era Span'}
        </div>
        {type === 'tl' ? (<>
        ...
        <div style={{ marginBottom: 10 }}>
          <label style={{ fontSize: '0.77em', color: 'var(--mut)', display: 'block', marginBottom: 3 }}>Color</label>
          <input type="color" value={form.col || '#c966ff'} onChange={e => set('col', e.target.value)} style={{ width: 44, height: 30, border: 'none', background: 'none', cursor: 'pointer' }} />
        </div>
        <div style={{ display: 'flex', gap: 8, justifyContent: 'space-between', marginTop: 6 }}>
          <div>{item.id && onDelete && (
            <button className="btn btn-outline btn-sm" style={{ color: '#ff3355', borderColor: '#ff3355' }}
              onClick={() => { if (window.confirm('Delete this row?')) { onDelete(item.id); onClose() } }}>Delete</button>)}
          </div>
        ...
        <div style={{ fontSize: '0.69em', color: 'var(--mut)', marginTop: 10 }}>
          Saved to the database like every other entry — no more disappearing eras.
        </div>
      </div>
    </div>
  )
...
export default function ErasView({ db }) {
  const rows = db.db.eras || []
  const timeline = rows.filter(r => r.kind === 'tl').sort((a, b) => (a.ord ?? 0) - (b.ord ?? 0))
  const spans = rows.filter(r => r.kind === 'span').sort((a, b) => (a.ord ?? 0) - (b.ord ?? 0))
  const [edit, setEdit] = useState(null) // { item, type }
  const seeded = useRef(false)

  // One-time seed/migration
  useEffect(() => {
    if (seeded.current || db.loading) return
    if (timeline.length || spans.length) { seeded.current = true; return }
    seeded.current = true
    let tl = null, sp = null
    try { const s = db.getSetting?.('eras_timeline'); if (s) tl = JSON.parse(s) } catch {}
    try { const s = db.getSetting?.('eras_spans'); if (s) sp = JSON.parse(s) } catch {}
    ;(tl || ERA_TIMELINE || []).forEach((e, i) => db.upsertEntry('eras', { id: 'era_tl_' + i + '_' + uid(), kind: 'tl', ord: i, ...e }))
    ;(sp || ERA_SPANS || []).forEach((s, i) => db.upsertEntry('eras', { id: 'era_span_' + i + '_' + uid(), kind: 'span', ord: i, ...s }))
  }, [db.loading])

  const save = type => form => {
    const entry = { kind: type, ord: form.ord ?? (type === 'tl' ? timeline.length : spans.length), ...form }
    if (!entry.id) entry.id = 'era_' + type + '_' + uid()
    db.upsertEntry('eras', entry)
  }
  const del = id => db.deleteEntry('eras', id)
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 6, marginBottom: 8 }}>
        {spans.map(s => (
          <div key={s.id} style={{ background: 'var(--card)', border: '1px solid var(--brd)', borderTop: `3px solid ${s.col || '#c966ff'}`, borderRadius: 'var(--r)', padding: 10, textAlign: 'center', position: 'relative' }}>
            <button onClick={() => setEdit({ item: s, type: 'span' })}
              style={{ position: 'absolute', top: 6, right: 6, background: 'none', border: 'none', color: 'var(--mut)', fontSize: '0.92em', cursor: 'pointer', padding: '2px 5px' }} title="Edit">✎</button>
            <div style={{ fontFamily: "'Cinzel', serif", fontSize: '1em', color: s.col || '#c966ff' }}>{s.name}</div>
      ...
      <button className="btn btn-sm btn-outline" style={{ marginBottom: 16 }} onClick={() => setEdit({ item: { col: '#c966ff' }, type: 'span' })}>+ Add Era Span</button>

      {/* Visual timeline */}
      <div style={{ background: 'var(--card)', border: '1px solid var(--brd)', borderRadius: 'var(--rl)', padding: 16, marginBottom: 16 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
          <div style={{ fontFamily: "'Cinzel', serif", fontSize: '1.08em', color: 'var(--cca)' }}>Timeline</div>
          <button className="btn btn-sm btn-outline" onClick={() => setEdit({ item: { col: '#c966ff' }, type: 'tl' })}>+ Add</button>
        </div>
        <div style={{ position: 'relative', paddingLeft: 90 }}>
          {timeline.map((e, i) => {
            const isLast = i === timeline.length - 1
            return (
              <div key={e.id} style={{ display: 'flex', minHeight: i === 0 ? 48 : 56, position: 'relative' }}>
                ...
                <div style={{ position: 'relative', width: 14, flexShrink: 0 }}>
                  <div style={{ width: 10, height: 10, borderRadius: '50%', background: e.col || '#c966ff', position: 'absolute', top: 3, left: 2, zIndex: 1 }} />
                  {!isLast && <div style={{ position: 'absolute', left: 6, top: 13, bottom: 0, width: 2, background: e.col || '#c966ff', opacity: .4 }} />}
                </div>
                <div style={{ padding: '0 0 12px 8px', flex: 1, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <div>
                    <div style={{ fontSize: '0.92em', fontWeight: 600, color: e.col || '#c966ff' }}>{e.label}</div>
                    <div style={{ fontSize: '0.85em', color: 'var(--dim)', marginTop: 1 }}>{e.desc}</div>
                  </div>
                  <button onClick={() => setEdit({ item: e, type: 'tl' })}
      ...
      {edit && (
        <EditModal item={edit.item} type={edit.type}
          onSave={save(edit.type)} onDelete={del} onClose={() => setEdit(null)} />
      )}
    </div>
  )
}
