...
export default function Wardrobe({ db }) {
  ...
  const [colSize, setColSize] = useState(() => {
    try { return localStorage.getItem('colsize_wardrobe') || 'M' } catch { return 'M' }
  })
          ...
          {['XS','S','M','L','XL'].map(sz => (
            <button key={sz} onClick={() => changeColSize(sz)}
              style={{ fontSize: '0.69em', padding: '2px 7px', borderRadius: 8, cursor: 'pointer',
                background: colSize === sz ? '#4361ee' : 'none',
                color: colSize === sz ? '#fff' : 'var(--dim)',
                border: `1px solid ${colSize === sz ? '#4361ee' : 'var(--brd)'}` }}>{sz}</button>
          ))}
        </div>
