...
function sessionColor(sessionNum, dateFallback) {
  // Try parsing session number directly
  let num = parseInt(sessionNum)
  // If that fails, try extracting number from strings like "Session 10"
  if (isNaN(num) || num === 0) {
    const match = String(sessionNum || '').match(/\d+/)
    if (match) num = parseInt(match[0])
  }
  // If still no number, derive from date string (e.g. "2026-03-24" -> use day+month)
  if (isNaN(num) || num === 0) {
    const d = String(dateFallback || '')
    const digits = d.replace(/\D/g, '')
    num = digits ? (parseInt(digits.slice(-4)) % 30) + 1 : 1
  }
  const hue = (330 + (num * 47)) % 360
  return {
...
function SessionCard({ session, onEdit, onDelete, selected, onSelect }) {
  ...
  const col = sessionColor(session.session_number, session.date)
  ...
  return (
    <div style={{
      ...
      >
        ...
        />
        <div style={{ flex: 1 }}>
          <div style={{ fontFamily: "'Cinzel',serif", fontSize: 'var(--fs-md)', fontWeight: 700, color: col.text }}>
            Session {session.session_number} · {session.date}
          </div>
          {session.topics && (
            <div style={{ fontSize: 'var(--fs-sm)', color: col.text, opacity: 0.75, marginTop: 2 }}>
              {session.topics}
            </div>
          )}
        </div>
        <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
          <button
            ...
            style={{ fontSize: 'var(--fs-sm)', padding: '2px 8px', borderRadius: 4, border: `1px solid ${col.border}`, background: 'none', color: col.text, cursor: 'pointer' }}
          >Edit</button>
          <button
            onClick={e => { e.stopPropagation(); onDelete(session.id) }}
            style={{ fontSize: 'var(--fs-sm)', padding: '2px 8px', borderRadius: 4, border: '1px solid #cc4444', background: 'none', color: '#cc4444', cursor: 'pointer' }}
          >✕</button>
          <span style={{ color: col.text, fontSize: 'var(--fs-md)', opacity: 0.6 }}>{expanded ? '▲' : '▼'}</span>
        </div>
      </div>
          ...
          {SECTION_LABELS.map(({ k, l }) => {
            ...
            return (
              <div key={k} style={{ marginTop: 10 }}>
                <div style={{ fontSize: 'var(--fs-xs)', fontWeight: 700, color: col.text, textTransform: 'uppercase', letterSpacing: '.06em', marginBottom: 4 }}>{l}</div>
                <div style={{ fontSize: 'var(--fs-sm)', color: col.text, whiteSpace: 'pre-wrap', lineHeight: 1.6, opacity: 0.9 }}>{session[k]}</div>
              </div>
            )
          })}
        </div>
...
function SessionForm({ initial, onSave, onCancel }) {
  ...
  const inputStyle = {
    ...
    color: 'var(--tx)', fontSize: 'var(--fs-sm)', outline: 'none', boxSizing: 'border-box',
  }
  ...
  return (
    <div style={{
      ...
      <div style={{ fontFamily: "'Cinzel',serif", fontSize: 'var(--fs-md)', color: col.text, marginBottom: 12, fontWeight: 700 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 8, marginBottom: 10 }}>
        <div>
          <div style={{ fontSize: 'var(--fs-xs)', color: col.text, marginBottom: 3, textTransform: 'uppercase', letterSpacing: '.04em' }}>Session #</div>
          <input type="number" value={form.session_number} onChange={e => set('session_number', e.target.value)} style={inputStyle} />
        </div>
        <div>
          <div style={{ fontSize: 'var(--fs-xs)', color: col.text, marginBottom: 3, textTransform: 'uppercase', letterSpacing: '.04em' }}>Date</div>
          <input type="date" value={form.date} onChange={e => set('date', e.target.value)} style={inputStyle} />
        </div>
        <div>
          <div style={{ fontSize: 'var(--fs-xs)', color: col.text, marginBottom: 3, textTransform: 'uppercase', letterSpacing: '.04em' }}>Element</div>
        ...
        <div>
          <div style={{ fontSize: 'var(--fs-xs)', color: col.text, marginBottom: 3, textTransform: 'uppercase', letterSpacing: '.04em' }}>Opened</div>
          <input value={form.opened_at} onChange={e => set('opened_at', e.target.value)} placeholder="e.g. 9:43 AM CDT" style={inputStyle} />
        </div>
        <div>
          <div style={{ fontSize: 'var(--fs-xs)', color: col.text, marginBottom: 3, textTransform: 'uppercase', letterSpacing: '.04em' }}>Closed</div>
      ...
      <div style={{ marginBottom: 10 }}>
        <div style={{ fontSize: 'var(--fs-xs)', color: col.text, marginBottom: 3, textTransform: 'uppercase', letterSpacing: '.04em' }}>Topics (brief summary line)</div>
      ...
      {SECTION_LABELS.map(({ k, l }) => (
        <div key={k} style={{ marginBottom: 8 }}>
          <div style={{ fontSize: 'var(--fs-xs)', color: col.text, marginBottom: 3, textTransform: 'uppercase', letterSpacing: '.04em' }}>{l}</div>
          <textarea
            value={form[k]}
      ...
      <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 12 }}>
        <button onClick={onCancel} style={{ padding: '6px 16px', borderRadius: 6, border: '1px solid var(--brd)', background: 'none', color: 'var(--dim)', cursor: 'pointer', fontSize: 'var(--fs-sm)' }}>Cancel</button>
        <button onClick={() => onSave(form)} style={{ padding: '6px 16px', borderRadius: 6, border: 'none', background: col.border, color: '#fff', cursor: 'pointer', fontSize: 'var(--fs-sm)', fontWeight: 700 }}>Save Session</button>
      </div>
    </div>
  )
}
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14, flexWrap: 'wrap', gap: 8 }}>
        <div style={{ fontFamily: "'Cinzel',serif", fontSize: 'var(--fs-md)', color: 'var(--cc)' }}>
          📋 Session Log
        </div>
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
          {selected.size > 0 && (
            <>
              <button onClick={exportSelected} style={{ fontSize: 'var(--fs-sm)', padding: '4px 12px', borderRadius: 8, border: '1px solid var(--cc)', background: 'none', color: 'var(--cc)', cursor: 'pointer' }}>
                ↓ Export {selected.size} selected
              </button>
              <button onClick={clearSelection} style={{ fontSize: 'var(--fs-sm)', padding: '4px 10px', borderRadius: 8, border: '1px solid var(--brd)', background: 'none', color: 'var(--dim)', cursor: 'pointer' }}>
                Clear
              </button>
            </>
          )}
          <button onClick={exportAll} style={{ fontSize: 'var(--fs-sm)', padding: '4px 12px', borderRadius: 8, border: '1px solid var(--brd)', background: 'none', color: 'var(--dim)', cursor: 'pointer' }}>
            ↓ Export All
          </button>
          <button
            onClick={() => { setAdding(true); setEditing(null) }}
            style={{ fontSize: 'var(--fs-sm)', padding: '4px 12px', borderRadius: 8, border: 'none', background: 'var(--cc)', color: '#000', cursor: 'pointer', fontWeight: 700 }}
          >
            + New Session
          </button>
        </div>
      ...
      {/* Flash message */}
      {msg && (
        <div style={{ fontSize: 'var(--fs-sm)', color: 'var(--cc)', marginBottom: 10, padding: '6px 12px', background: 'var(--card)', borderRadius: 6, border: '1px solid var(--brd)' }}>
          {msg}
        </div>
      )}
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 12, flexWrap: 'wrap', alignItems: 'center' }}>
        <input
          ...
          style={{ flex: 1, minWidth: 160, padding: '6px 10px', borderRadius: 6, border: '1px solid var(--brd)', background: 'var(--sf)', color: 'var(--tx)', fontSize: 'var(--fs-sm)', outline: 'none' }}
        />
          ...
          {['All', ...ELEMENT_OPTS].map(el => {
            ...
            return (
              <button
                ...
                style={{
                  fontSize: 'var(--fs-xs)', padding: '3px 9px', borderRadius: 12, cursor: 'pointer',
        ...
        {filtered.length > 0 && (
          <button onClick={selectAll} style={{ fontSize: 'var(--fs-xs)', padding: '3px 9px', borderRadius: 12, border: '1px solid var(--brd)', background: 'none', color: 'var(--dim)', cursor: 'pointer' }}>
            Select all visible
          </button>
        )}
      </div>
      ...
      {/* Session list */}
      {filtered.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '40px 20px', color: 'var(--dim)', fontStyle: 'italic', fontSize: 'var(--fs-md)' }}>
      ...
      <div style={{ fontSize: 'var(--fs-sm)', color: 'var(--dim)', textAlign: 'center', marginTop: 12 }}>
